...
const downloadAllAudio = async () => {
  ...
  for (const char of hiraganaData) {
    const outputPath = path.join('public', 'sounds', 'hiragana', `${char.romaji}.mp3`);
    await downloadAudio(char.character, outputPath);
  ...
  for (const char of katakanaData) {
    const outputPath = path.join('public', 'sounds', 'katakana', `${char.romaji}.mp3`);
    await downloadAudio(char.character, outputPath);
    // Esperar 1 segundo entre descargas para no sobrecargar la API
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
